...
def loginUser(request):
    if not request.user.is_authenticated:
        if request.method == "POST":
            ...
            if not request.POST.get('email').endswith("@kiwitech.com"):
                messages.error(request, "Email those ends with @kiwitech.com only are allowed")
...
def signup(request):
    if request.method == 'POST':
        ...
        if not request.POST.get('email').endswith("@kiwitech.com"):
            messages.error(request, "Email ends with @kiwitech.com only are allowed")
            return render(request, "signup.html")
        if password == password_c:
            try:
                user = User.objects.create_user(email.strip().lower(), email.strip().lower(), password)
                user.first_name = ' '.join(i.capitalize() for i in name.strip().split(' '))
                user.save()
                login(request, user)
                messages.success(request, "Logged In Successfully")
                return redirect("home")
...
def fetch(request):
    post_list= Paginator(Post.objects.all().order_by('-created_on'), 10)
